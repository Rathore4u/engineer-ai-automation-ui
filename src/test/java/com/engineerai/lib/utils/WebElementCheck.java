	...
	*/
	...
	public void assertElementText(final WebElement element, final String expectedText)
	{
		...
		if (actualText != null)
			...
			Assert.fail("FAIL: Element's Text is null");
	}

	public void assertElementTextContains(final WebElement element, final String expectedText)
	{
		final String message = String.format("FAIL : Element is not having this text '%s'", expectedText);
		final String actualText = element.getText();
		if (actualText != null)
			Assert.assertTrue(message, actualText.contains(expectedText));
		else
			Assert.fail("FAIL: Element's Text is null");
	}
